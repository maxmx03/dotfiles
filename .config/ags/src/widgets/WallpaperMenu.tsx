...
export default function WallpaperMenu({ onBack }: { onBack: () => void }) {
  ...
  return (
    ...
    >
      <box spacing={8}>
        <button class="arrow-button" onClicked={onBack}>
        ...
        <button
          ...
          onClicked={() =>
            execAsync(`xdg-open "${WALLPAPER_DIR}"`).catch(console.error)
          }
        >
            ...
            <box spacing={8} homogeneous>
              {row.map((wp) => (
                <button
                  ...
                  tooltipText={wp}
                >
                  <box cssClasses={["wallpaper-preview"]} heightRequest={80}>
                    <image file={`${WALLPAPER_DIR}/${wp}`} pixelSize={80} />
                  </box>
                </button>
              ))}
